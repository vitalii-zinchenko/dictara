...
export function ApiKeys() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        console.log('[ApiKeys] Loaded config:', loadedConfig)
      } catch (e) {
        console.error('[ApiKeys] Failed to load data:', e)
      } finally {
        setIsLoading(false)
      }
    }
  ...
  const openaiForm = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        ...
        console.error('[ApiKeys] Failed to save OpenAI API key:', e)
  ...
  const azureForm = useForm({
    ...
    onSubmit: async ({ value }) => {
      console.log('[ApiKeys] Saving Azure configuration...')
      ...
      try {
        ...
        console.log('[ApiKeys] Azure configuration saved successfully')
        setAzureState((prev) => ({
          ...prev,
          existingKey: value.apiKey,
          saveSuccess: true,
        ...
        console.error('[ApiKeys] Failed to save Azure configuration:', e)
  ...
  const handleTestOpenAI = async () => {
    ...
    try {
      ...
      console.error('[ApiKeys] Failed to test OpenAI API key:', e)
      setOpenaiState((prev) => ({
        ...prev,
        testResult: 'error',
  ...
  const handleTestAzure = async () => {
    ...
    console.log('[ApiKeys] Testing Azure API key...')
    setIsTesting('azure')
    ...
    try {
      ...
      console.log('[ApiKeys] Azure API key test result:', isValid)
      setAzureState((prev) => ({
        ...prev,
        testResult: isValid ? 'success' : 'error',
      ...
      console.error('[ApiKeys] Failed to test Azure API key:', e)
      setAzureState((prev) => ({
        ...prev,
        testResult: 'error',
        errorMessage: `Test failed: ${e}`,
  ...
  const handleDeleteOpenAI = async () => {
    console.log('[ApiKeys] Deleting OpenAI API key...')
    try {
      await invoke('delete_openai_key')
      console.log('[ApiKeys] OpenAI API key deleted successfully')
      setOpenaiState({
        existingKey: null,
        testResult: null,
        saveSuccess: false,
      ...
      console.error('[ApiKeys] Failed to delete OpenAI API key:', e)
      setOpenaiState((prev) => ({ ...prev, errorMessage: `Failed to delete: ${e}` }))
    }
  }

  const handleDeleteAzure = async () => {
    console.log('[ApiKeys] Deleting Azure API key...')
    try {
      await invoke('delete_azure_key')
      console.log('[ApiKeys] Azure API key deleted successfully')
      setAzureState({
        existingKey: null,
        testResult: null,
        saveSuccess: false,
      ...
      console.error('[ApiKeys] Failed to delete Azure API key:', e)
  ...
  const handleToggleProvider = async (provider: Provider) => {
    ...
    try {
      ...
      console.log('[ApiKeys] Provider config updated:', newProvider)
    } catch (e) {
      console.error('[ApiKeys] Failed to update provider config:', e)
    }
  }
